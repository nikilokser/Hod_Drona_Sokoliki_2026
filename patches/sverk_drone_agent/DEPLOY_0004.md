...
Пошаговая инструкция для bishopl-108, bishopr-4, knightr-6, knightl-107,
queen-111 (актуальный ростер по `sverk_ai_communication_server/config/
fleet.yaml` на 2026-08-01 — **не** старые имена `sverk-108`/`sverk-4`/
`sverk-6`/`drone-05`/`drone-06`, они больше не соответствуют реальным
роботам). Патч 0004 самодостаточен — не требует предварительного применения
0001-0003 (они чинят другое: таймауты полётных команд).

**Важно:** IP из README устарели у части дронов (третий октет сменился с `.1`
на `.12`) — уточните актуальный IP каждого дрона перед подключением
(`fleet.yaml` или `GET /api/robots` нашего веб-бэкенда — надёжнее таблицы в
README.md).

**Важно (обновление, не первая установка):** по имеющейся информации, более
РАННЯЯ версия патча (без выноса LLM-вызова в подпроцесс, менее надёжная) уже
была развёрнута кем-то на всех дронах ниже. Шаг 0 покажет, какая версия
реально стоит — если это ранняя, накатите файлы из этого репозитория поверх,
это обновление, а не первая установка (шаги те же).
...
ls -la ~/catkin_ws/src/sverk_drone_agent/ros1_ws/src/drone_pseudo_agent_ros1/scripts/vote_llm_call.py 2>&1
grep -n 'VOTE_MAX_ATTEMPTS\|VOTE_SUBPROCESS_TIMEOUT_SEC' ~/catkin_ws/src/sverk_drone_agent/ros1_ws/src/drone_pseudo_agent_ros1/scripts/pseudo_agent_text_node.py 2>&1
```

Должно быть `active (running)`. Три варианта по результатам:

- `vote_llm_call.py` **не существует** → патча 0004 нет вообще, это первая
  установка (шаги 1-6 ниже как есть).
- `vote_llm_call.py` есть, но `VOTE_MAX_ATTEMPTS` **не 5** (или `grep` вообще
  ничего не находит — значит стоит совсем ранняя, без-подпроцессная версия)
  → это обновление, накатить файлы из репозитория поверх (шаги 2-3 ниже,
  шаг 1 с ключом можно пропустить, если он уже стоит и работает).
- `VOTE_MAX_ATTEMPTS=5` уже стоит → патч актуален, ничего катить не надо,
  сразу к шагу 4 (проверка).

Если `DRONE_AGENT_MODE=pseudo` — патч работает как задуман (перехватывает
`[ГОЛОСОВАНИЕ]` до regex-парсера). Если уже `agent` — патч всё равно
безопасен (просто не понадобится regex-обход), но тогда голосование,
...
при нестабильной сети может занять до ~42 с из-за встроенных повторов — это
