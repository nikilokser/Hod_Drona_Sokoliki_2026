...
`VOTE_MAX_ATTEMPTS=5`, `VOTE_SUBPROCESS_TIMEOUT_SEC=8`,
`VOTE_RETRY_DELAY_SEC=0.5` - худший случай ~42 с, ~18 с запаса под потолком
в 60 с (было `3/13/1` → ~41 с; после ужатия таймаута попыток стало больше на
тот же бюджет - таймаут короткий намеренно, см. комментарий в коде: реальный ответ
всегда быстрый (~0.15-4 с), долгий запрос - не «вот-вот ответит», а плохой
запрос, короче таймаут = больше попыток за тот же бюджет = раунд заканчивается
быстрее в обе стороны).
...
**Ростер и имена сменились** с изначального `sverk-N` на имена по роли фигуры
(проверено по `fleet.yaml` 2026-08-01) - старые `sverk-108`/`sverk-4`/
`sverk-6`/`drone-05`/`drone-06` из более ранней версии этой таблицы **не
соответствуют** текущему ростеру, не используйте их.

**Статус 0004 на 2026-08-01:** по словам команды, патч (более ранняя,
не-subprocess версия, до фикса надёжности LLM-вызова) был **уже развёрнут
кем-то другим на всех дронах ниже до этого фикса** - то есть все они,
скорее всего, нуждаются в обновлении до текущей версии (subprocess +
`VOTE_MAX_ATTEMPTS=5`), не в первой установке. Проверить фактическую версию
на конкретном дроне перед решением, накатывать ли (см. `DEPLOY_0004.md`,
шаг 0). Сам я смог применить/проверить только на `king-8` - остальные пять
были не в сети (offline по Gateway) в момент проверки, IP не пингуются:

| robot_id     | IP             | 0001                | 0002                | 0003                | 0004                |
|--------------|----------------|----------------------|----------------------|----------------------|----------------------|
| king-8       | 192.168.12.8   | применён 2026-07-30  | применён 2026-07-30  | применён 2026-07-30  | применён 2026-08-01 (текущая subprocess-версия, 5 попыток), проверен полным боевым путём |
| bishopl-108  | 192.168.12.108 | ?                     | ?                     | ?                     | по словам команды - развёрнута более ранняя версия, нужно обновить; не проверено (offline 2026-08-01) |
| bishopr-4    | 192.168.12.4   | ?                     | ?                     | ?                     | то же |
| knightr-6    | 192.168.12.6   | ?                     | ?                     | ?                     | то же |
| knightl-107  | 192.168.12.107 | ?                     | ?                     | ?                     | то же |
| queen-111    | 192.168.12.111 | ?                     | ?                     | ?                     | то же |
