...
### Ожидает раскатки (2026-08-02) — заметка про очки в `VOTE_SYSTEM_PROMPT`

Бэкенд (`web/backend/move_orchestrator.py`) теперь учит модель, предлагающую
ход, что матч решают очки, а не мат, и что ради выгодных очков иногда стоит
играть не самым сильным позиционно ходом. Голосующий промпт на дронах
(`VOTE_SYSTEM_PROMPT` в `pseudo_agent_text_node.py`) об этом ничего не знал —
получал только обоснование предложенного хода текстом, без своих правил его
оценить, и мог заветировать разумный по очкам, но позиционно спорный ход.

`pseudo_agent_text_node.py.patched` в этой папке уже обновлён (см. git-diff
файла) — добавлен абзац в `VOTE_SYSTEM_PROMPT`, объясняющий то же самое
голосующему агенту. **Не раскатано ни на одном дроне** — `king-8`
недоступен на момент этого изменения (все аппараты offline), диф против
реального бэкапа на устройстве перегенерировать не удалось. Когда дрон
снова будет в сети:

1. Сверить `pseudo_agent_text_node.py.patched` в этой папке с тем, что
   реально стоит на дроне (`diff` как в шаге "Как применить" ниже).
2. Если абзаца про очки там нет — докопировать блок `VOTE_SYSTEM_PROMPT`
   целиком из `.patched`-файла, перезапустить сервис.
3. Перегенерировать `0004-route-voting-prefix-to-llm-in-pseudo-mode.patch`
   от актуального бэкапа на устройстве, чтобы патч-файл снова отражал
   реально применённое состояние.
